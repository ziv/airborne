...
export class ScenarioState extends EventBus {
  ...
  addEntity(type, worldX, worldZ, overrides = {}) {
    ...
    const properties = {};
    if (def.properties) {
      for (const p of def.properties) properties[p.key] = p.default;
    }
  ...
  updateEntity(id, changes) {
    ...
    for (const [key, value] of Object.entries(changes)) {
      if (key === 'position') {
        ...
        Object.assign(entity.properties, value);
      } else {
        entity[key] = value;
      }
    }
  ...
  addObjective(data = {}) {
    const id = data.id || `obj-${String(this.scenario.objectives.length + 1).padStart(2, '0')}`;
    const obj = {
      id,
      type: 'destroy',
      target: '',
