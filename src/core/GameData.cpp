...
bool GameData::isGoodLanding() const {
    ...
    // decompose quaternion into pitch / roll angles
    const Vector3 euler = QuaternionToEuler(state.orientation.rotation);
    const float pitchDeg = euler.x * RAD2DEG;
    const float rollDeg  = euler.z * RAD2DEG;
    ...
    if (!wingsLevel) {
        TraceLog(LOG_ERROR, "Landing failed: wings not level (roll = %.1f°)", rollDeg);
    }
    if (!noseOk) {
        TraceLog(LOG_ERROR, "Landing failed: bad pitch angle (pitch = %.1f°)", pitchDeg);
    }
    if (!slowEnough) {
        TraceLog(LOG_ERROR, "Landing failed: too fast (speed = %.1f m/s)", state.forces.speed);
    }
