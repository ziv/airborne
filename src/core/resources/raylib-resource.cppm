...
class RaylibResource {
 ...
 public:
  ...
  ~RaylibResource() {
    if (has_ownership) {
      // why it safe to run the unloader without checking
      // the resource itself?
      // fortunate, raylib does this check for us and
      // silently ignore the unload if resource
      // does not exist
      Unloader(res);
    }
  }
  ...
  RaylibResource(RaylibResource &&other) noexcept : res(other.res), has_ownership(other.has_ownership) { other.has_ownership = false; }
