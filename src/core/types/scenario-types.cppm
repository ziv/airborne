...
export {
  ...
  NLOHMANN_JSON_SERIALIZE_ENUM(Faction, {{Faction::FRIENDLY, "friendly"},
                                         {Faction::ENEMY, "enemy"},
  ...
  NLOHMANN_JSON_SERIALIZE_ENUM(Weather, {{Weather::SUNNY, "sunny"},
  ...
  NLOHMANN_JSON_SERIALIZE_ENUM(Season, {{Season::SUMMER, "summer"},
                                        {Season::WINTER, "winter"}});

  enum class TimeOfDay { DAY, NIGHT, DAWN, DUSK };

  NLOHMANN_JSON_SERIALIZE_ENUM(TimeOfDay, {{TimeOfDay::DAY, "day"},
  ...
  NLOHMANN_JSON_SERIALIZE_ENUM(WeaponCategory,
                               {{WeaponCategory::AIR_TO_AIR, "air-to-air"},
  ...
  NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(WeaponAgility, maxPitch,
                                                  maxYaw, maxRoll);
  ...
  NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(WeaponEffects, areaOfEffect,
                                                  armorPenetration);
  ...
  struct WeaponDef {
    ...
    float rateOfFire =
        1.0f; ///< Rounds per second (guns) or launches per second.
    ...
    bool playerSelectable =
        false; ///< Available in the player weapon selection screen.
    bool playerDefault = false; ///< Pre-selected in the default loadout.
  };

  NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(
      WeaponDef, id, name, category, range, damage, ammoCapacity, rateOfFire,
      speed, maxVelocity, agility, effects, playerSelectable, playerDefault);
  ...
  struct EntityDef {
    ...
    std::string
        subtype; ///< Optional subclassification (e.g. "carrier" for AIRBASE).
    Faction faction = Faction::ENEMY;
    EntityState state = EntityState::ACTIVE;

    Vector3 position = (Vector3){
        0, 0, 0}; ///< World-space position (before large-world offset).
    ...
    std::string
        modelId; ///< Path to the .glb model file, or empty for fallback cube.
    ...
    [[nodiscard]] float getParam(const std::string &key,
                                 const float defaultValue = 0.0f) const {
  ...
  NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(EntityDef, id, type, subtype,
                                                  faction, state, position,
                                                  heading, health, maxHealth,
                                                  modelId, params, properties,
                                                  weapons, waypoints);
  ...
  struct Objective {
    ...
    bool hidden =
        false; ///< If true, revealed during gameplay rather than at briefing.
  };

  NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(Objective, id, type, target,
