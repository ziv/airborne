/**
 * @file Resource.h
 * @brief RAII wrappers for raylib GPU resources (textures, models, shaders, audio).
 *
 * `RaylibResource<T, Unloader>` ensures the correct unload function is called
 * when the wrapper goes out of scope. Move-only semantics prevent accidental
 * double-free of GPU handles.
 */
#pragma once
#include "raylib.h"
