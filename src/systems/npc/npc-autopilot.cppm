...
void autopilot(entt::registry& registry, const float dt) {
  ...
  const auto view = registry.view<NpcTag, AiController, AircraftOrientation,
                                   PhysicsProfile, Position3D, Velocity3D>();
  ...
  for (auto [entity, ai, orient, profile, pos, vel] : view.each()) {
    ...
    switch (ai.state) {
      ...
      // ---- PATROL ----------------------------------------------------------
      case AiState::PATROL: {
        const auto* waypoints = registry.try_get<Waypoints>(entity);
        if (waypoints && !waypoints->waypoints.empty()) {
          const Vector3& wp = waypoints->waypoints[ai.waypoint_index];
          const Vector3 to_wp = wp - npc_abs;
          const float dist = Vector3Length(to_wp);
      ...
      case AiState::ENGAGE: {
        ...
        if (const auto* weapons = registry.try_get<Weapons>(entity)) {
          ...
          for (const auto& [name, count] : weapons->weapons) {
            if (count > 0) { out_of_missiles = false; break; }
          }
        }
        ...
        // Steer toward player
        if (dist > 0.1f)
          ai.desired_dir = Vector3Normalize(to_player);
          ...
          if (angle_cos > 0.97f) {  // ~14 degree cone
            ...
            if (auto* weapons = registry.try_get<Weapons>(entity)) {
              for (auto& [name, count] : weapons->weapons) {
                if (count > 0) {
                  --count;
                  const float rpm = registry.try_get<WeaponLauncher>(entity)
                                    ? static_cast<float>(registry.get<WeaponLauncher>(entity).fire_rate_rpm)
                                    : 20.0f;
      ...
      case AiState::DISENGAGE: {
        ...
        const float dist = Vector3Length(to_player);
        if (dist > 0.1f)
          ai.desired_dir = Vector3Normalize(Vector3Negate(to_player));  // fly away
