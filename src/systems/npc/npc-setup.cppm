...
void setup(entt::registry& registry) {
  ...
  for (const auto [entity, profile, heading, vel] : view.each()) {
    ...
    TraceLog(LOG_DEBUG, "Setting up NPC %d with cruise speed %.1f m/s", static_cast<int>(entity), cruise);
    registry.emplace<AiController>(entity, AiState::PATROL, 0, entt::entity{entt::null}, 0.0f, 0.0f, fwd, cruise);
  }
}
