...
void physics(entt::registry& registry, const float dt) {
  ...
  const float speed_ratio = std::clamp(player.speed / (conf.max_speed * 0.8f), 0.0f, 1.2f);
  const float control_authority = speed_ratio * speed_ratio;

  // on ground, we allow user to yaw
  const float yaw_authority = flying ? control_authority : conf.min_authority;

  // some effects...

  // slip, bank angle induces a yaw moment toward the lowered wing.
  const float induced_yaw = player.right.y * conf.bank_induce_yaw_ratio;
  ...
  const float induced_pitch = -lift_loss * conf.lift_loss_pitch_ratio;

  // all together
  const Vector3 pitch_torque = Vector3Scale(player.right, (inputs.pitch * conf.pitch_ratio * control_authority + induced_pitch) * dt);
  const Vector3 roll_torque = Vector3Scale(player.forward, inputs.roll * conf.roll_ratio * control_authority * dt);
  const Vector3 yaw_torque = Vector3Scale(player.up, (inputs.yaw * conf.yaw_ratio * yaw_authority + induced_yaw) * dt);

  player.angular_velocity = player.angular_velocity + pitch_torque + roll_torque + yaw_torque;

  // some air dumping
  float damping_factor = 1.0f - conf.air_damping_factor * dt;
  ...
  Ratio cd = conf.drag_coefficient;
  if (inputs.brakes) cd += cd * 3.0f;
  if (inputs.gear) cd += cd * 3.0f;

  const float drag = square_speed * cd;
  const float thrust = inputs.throttle * conf.engine_thrust;
  const float lift = square_speed * conf.lift_coefficient;
  ...
  if (!flying) {
    ...
    if (float ground_speed = Vector3Length(ground_velocity); ground_speed > 0.1f) {
      if (const auto lz = get_landing_zone(registry); lz.is_carrier && inputs.brakes) {
  ...
  // force vectors
  const Vector3 thrust_force = player.forward * thrust;
  const Vector3 drag_force = player.forward * -drag;
  const Vector3 lift_force = player.up * lift;
  const Vector3 weight_force = gravity() * mass;

  const Vector3 total = thrust_force + drag_force + weight_force + lift_force;
  const Vector3 acceleration = total * 1 / mass;

  player.velocity = player.velocity + acceleration * dt;
  player.speed = Vector3Length(player.velocity);

  // hard speed cap (normally drag balances thrust before this limit)
  // if (player.speed > conf.max_speed && player.speed != 0.0f) {
  //   player.velocity = player.velocity * conf.max_speed / player.speed;
  //   player.speed = conf.max_speed;
  // }
  ...
  if (player.speed > conf.stall_speed) {
