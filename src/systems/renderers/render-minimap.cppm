...
static float minimap_tile_size(const int zoom) {
    return 9783.9f * std::pow(2.0f, 12 - zoom);
}

export void RenderMinimap(entt::registry &registry) {
  const auto view = registry.view<DashboardSlot, MinimapWidget, Position2D>();
  ...
  for (auto [entity, slot, wd, pos] : view.each()) {
    ...
    Camera2D cam{};
    cam.target   = { (abs_pos.x + origin_x) / tile_sz * tile_px,
                     (abs_pos.z + origin_z) / tile_sz * tile_px };
    cam.zoom     = 1.0f;
    ...
    for (const auto tile_view = registry.view<MapTile>(); const auto [te, tile] : tile_view.each()) {
      ...
      DrawTexture(tex, tile.geo_x * static_cast<int>(tile_px),
                       tile.geo_z * static_cast<int>(tile_px), WHITE);
    }
