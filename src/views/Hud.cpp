...
void Hud::Draw(const GameCamera &camera, const Orientation &orientation) const {
    BeginScissorMode(HudX, HudY, HudSize, HudSize);
    BeginScissorMode(HudX, HudY, HudSize, HudSize);

    const Camera rayCam = camera.GetRaylibCamera();
    const Vector3 camForward = Vector3Normalize(Vector3Subtract(rayCam.target, camera.GetPosition()));
    ...
    const Vector3 refPt = Vector3Add(camera.GetPosition(), Vector3Scale(camForward, 10000.0f));
    ...
    DrawText(TextFormat("%0.f", orientation.Speed), HudX + 10, HudY + HudSize / 2, 15, GREEN);
    DrawText(TextFormat("%0.f", camera.GetAltitude()), HudX + HudSize - 30, HudY + HudSize / 2, 15, GREEN);
    ...
    for (int angle = -80; angle <= 80; angle += 20) {
        ...
        const Vector3 center3D = Vector3Add(camera.GetPosition(), Vector3Scale(rungDir, 10000.0f));
        const Vector3 side3D = Vector3Add(center3D, Vector3Scale(flatRight, 500.0f));

        const Vector2 center = GetWorldToScreenEx(center3D, rayCam, GameConfig::SCREEN_WIDTH, GameConfig::SCREEN_HEIGHT);
