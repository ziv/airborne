/**
 * @file NavballView.cpp
 * @brief 3D navball off-screen rendering and compositing.
 */
...
void NavballView::draw(const AircraftState &state, const Vector2 &center) {
    ...
    EndMode3D();
    EndTextureMode();
    const Rectangle src = {0.0f, 0.0f, static_cast<float>(vtex.texture.width), -static_cast<float>(vtex.texture.height)};
    DrawTextureRec(vtex.texture, src, {center.x - 100.0f, center.y - 95.0f}, WHITE);
